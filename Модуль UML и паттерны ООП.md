...
| sort() | Клиент запрашивает сортировку, Context делегирует сортировку выбранной стратегии
|===
endlegend
...
2. **Класс `WeatherStation`** (реализует `Subject`):
    ...
    - Имеет метод `measurementsChanged()`, который вызывается при обновлении данных о погоде и в свою очередь вызывает `notifyObservers()`.
...
WS -> Obs : registerObserver(CCD)
WS -> Obs : registerObserver(SD)
WS -> WS : measurementsChanged()
...
doc -> pdf : visitTextDocument(this)

legend right
|===
| **Действие** | **Описание**
| visitTextDocument(this) | Документ принимает посетителя для экспорта в PDF
|===
endlegend
...
PlayerState <|-down- PlayingState
PlayerState <|-down- PausedState
PlayerState <|-down- StoppedState
