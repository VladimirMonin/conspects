...
PremissionRequiredMixin и декораторор PremissionRequired - что это?
Как это настраивается?
атрибут premission_requred = имя приложения.разрешение _ имя таблицы
Логика записей в этот атрибут с примерами
Добавив это, пользователи у которых нет прав будут видеть 403
Суперпользователю доступно абсолютно все - назначать ничего не надо
Как добавить созданное разрешение другому пользователю через админку?

Как работать с разрешениями в шаблонах
Как добавить в шаблон представления кнопку редактировать?
Колекция perms
perms.card.change_card - если есть, отобразим BS5 иконку карандаша со ссылкой на редактирование

На практике часто нужно выдавать разрешения группам
Группы можно создавать в админ панели
Создадим группу модератор
Дадим им разрешения
Присвоим пользователю эту группу
Теперь он может создавать CRUD операции с таблицей

Команды set add remove clear permissions
Как и где и когда этим можно и нужно пользоватся
Методы для работы с этим

Мы можем добаывать разрешения, добавлять их пользователю
Добавлять пользователя и исключать из категорий

Как работает протокол Oauth 2.0 на примере с вк?
Подробно пошагово от разрешения пользователя до получения доступа к своей учетке на сайте
Детально про каждый запрос который произойдёт

PythonSocialAuth
установка в виртуальное окружение 
добавление в файл зависимостей
добавление в install apps
после надо выполнить миграции (если работаем с постгрой, надо выполнить social_auth_jsonfield_enabled = ture)
миграции поставляются с этим пакетом надо их сделать
migrate

Надо прописать бекенд  GithubOauth2
Надо сделать маршруты social-auth social_django.urls namespace social

Чтобы взаимодействовать с социальным сервисом на своем сайте, нужно создать приложение
Для гитхаба это settings - developers settings - oauth apps - новое приложение
Название, url адрес (можем на гитхабе дать локалхост, но некоторые соцсети требуют именно адрес и https)
callback адрес social-auth/complete/github
После заполнения обязательных полей
мы увидим cleint id, а так же мы можем сгенерировать секретный ключ
прописываем в настройках открытый и закрытый ключ
social_auth_github_key
social_auth_github_key_secret

мы можем сделать ссылку на гитхаб авторизацию в шаблоне авторизации
url social:begin github - маршрут есть в документации

Как в акаунте пользователя перестать отображать кнопку сменить пароль
Мы можем создать группу social и в нее добавим разрешение Social Auth
Если пользователь авторизуется через социальную сеть попадает автоматом в эту группу
social auth pipline 
users.pipeline.new_user_handler
в приложении users pipline.py
def new_user_handler (backend, user, resonse, arg, kwark)
group = Group. filter name = social
if len group
user.groups.add(group0)
все пользователи которые зайдут через соцсети мы можем определить в группу

в шаблоне
если пользователи не имеют разрешения users.social_auth
им нужно показать ссылку смены пароля
