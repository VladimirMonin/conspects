...
| `URLValidator`                          | Проверяет, что строка является действительным URL.                                           | `URLValidator(message="Неверный формат URL.")`                                                                |
...
mixin для контекста
DataMixin - будет отвечать за наполнение шаблона стандартной информацией
создаем utlil.py и в нем DataMixin класс
def get_mixin_contex (self, context, kwarg)
context[menu] = menu
context.update kwargs

Добавим это в класс AddCardView
...
title_page = None
extra_context = {}
Инициализатор, если self.title_page: self.extra_context[title] = self.title_page
Соответственно в AddCardView надо добавить эти атрибуты
if "menu" not in self.extra_context: self.extra_context[menu] = menu