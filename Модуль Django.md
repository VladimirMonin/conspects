...
3. **Обработка формы в `views.py`**:
   ...
   - После успешного вход

а пользователь перенаправляется на указанную страницу (`some-view`). Если учетные данные неверны, в форму добавляется общая ошибка.
...
2. В файле `context_processors.py`, определите функцию `get_menu`, которая будет возвращать данные меню. Пример:
   ...
   ```python
   # Импортируем меню из вашего модуля, например menu.py
   from .menu import menu_items

   def get_menu(request):
       # Функция возвращает словарь, который будет доступен во всех шаблонах
       return {'mainmenu': menu_items}
   ```

   Где `menu_items` может быть определен как:
...
class AddCardCreateView(LoginRequiredMixin, CreateView):
    login_url = '/users/login/'  # Указываем собственный URL для перенаправления на вход
...
Переделаем нашу форму и наследуемся от этого
Далее мы должны определить username password1 password2
В полях тоже теперь password1
в мете делаем widjets = мы можем передать стили через него
Так же, можно убрать метод проверки емейла

Что такое методы clean_ в формах? Они вызываются автоматически?


Теперь можно сделать шаблон для отображения в цикле, а не as_p - как мы делали это в других формах

Класс вью для регистрации пользователя
Что лучше использовать? Специального нет для пользователей
Но отлично подходит CreateView
...
Прикольные анимации для авторизации https://iconscout.com/lottie-animations/authorization
