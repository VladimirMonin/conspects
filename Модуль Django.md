...
### Как избежать дублирующихся запросов
...
Опиши текст домашнего задания для студентов.
Ты должен дать подробные пошаговые описания действий, без больших блоков кода. 
Начни с краткого содержания домашнего задания. 
Потом само задание. 
Потом критерии проверки этого задания. 

Студенты должны будут кастомизировать Джанго Админку для работы со следующей моделью.
...
Студентам надо будет 
1. Подключить все модели к Админке используя классы и декоратор для подключения
2. Проверить отображение и работоспособность моделей
3. Модель CardTags, вероятно, вызовет проблемы, т.к. в ней нет поля id которое нужно для работы. Студентам надо будет добавить это, используя миграции. Или, добавить через SQLiteStudio, в таком случае, изменения с таблицами ДОЛЖНЫ БЫТЬ ОБЯЗАТЕЛЬНО В миграциях в том числе!
4. Студентам необходимо руссифицировать админку
5. Настроить choises для поля карточка. Это должно быть интеджер чойсес в булеан филд. Для конвертации значений они могут использовать этот код. `check_status = models.BooleanField(choices=tuple(map(lambda x: (bool(x[0]), x[1]), Status.choices)), default=Status.UNCHECKED, db_column='CheckStatus')`
6. Настроить кастомный фильтр, для карточек, одобрены модератором, и не одобрены
7. Добавить статус одобрения в отображение в админке, чтобы это было редактируемым полем, и из общего отображения карточек, можно было переключать статус одобрения.
